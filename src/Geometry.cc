...
void Geometry::GeometricModelCorrection(const ORB_SLAM2::Frame &currentFrame,
                                        cv::Mat &imDepth, cv::Mat &mask)     //多视图几何   输入的是当前帧  当前帧的深度  当前帧的yolact的mask
{
    ...
    else if (mDB.mNumElem >= ELEM_INITIAL_MAP)   //也就是数据库中的参考帧需要大于5
    {
        vector<ORB_SLAM2::Frame> vRefFrames = GetRefFrames(currentFrame);   //找到参考的关键帧  从数据库20帧中  选择5帧最接近的关键帧
        vector<DynKeyPoint> vDynPoints = ExtractDynPoints(vRefFrames,currentFrame);   //找到当前关键帧上的动态点
        mask = DepthRegionGrowing(vDynPoints,imDepth);                         //多视图几何映射到深度图上的mask   
        CombineMasks(currentFrame,mask);                          //这里是引用传参，maks会结合currentFrame中成员变量Mask  这里只是把两个mask放在一起了
...
void Geometry::GeometricModelUpdateDB(const ORB_SLAM2::Frame &currentFrame){
    if (currentFrame.mIsKeyFrame)   //只有关键帧才会被插入数据库
    {
...
vector<ORB_SLAM2::Frame> Geometry::GetRefFrames(const ORB_SLAM2::Frame &currentFrame){
    ...
    //距离和角度的欧氏距离   数据库中的历史20帧  存在 vDist 和  vRot中
    ...
    //归一化  将 vDist 和  vRot中数据归一化至[0-1]之间
...
//vRefFrames 这是从数据库中筛选的5帧最接近的关键帧  currentFrame 是当前帧
...
void Geometry::CombineMasks(const ORB_SLAM2::Frame &currentFrame, cv::Mat &mask)
{
    ...
    cv::Mat _mask = _maskL | _maskG;             //这里是把两个mask融合到一起
...
void Geometry::FillRGBD(const ORB_SLAM2::Frame &currentFrame,cv::Mat &mask,cv::Mat &imGray,cv::Mat &imDepth,cv::Mat &imRGB){
    ...
    cv::Mat imGrayAccumulator = imGray.mul(mask);   //灰度图像与mask求与操作  
    imGrayAccumulator.convertTo(imGrayAccumulator,CV_32F);
    
    
    cv::Mat bgr[3];                                 //RGB图像分为R G B分别与mask求与操作
    cv::split(imRGB,bgr);
    ...
    cv::Mat imCounter;                              //maks转化数据类型
    mask.convertTo(imCounter,CV_32F);
    
    cv::Mat imDepthAccumulator = imDepth.mul(imCounter);    //depth转化数据类型
    imDepthAccumulator.convertTo(imDepthAccumulator,CV_32F);
    ...
    cv::Mat K = cv::Mat::eye(3,3,CV_32F);            //相机的内参矩阵
    K.at<float>(0,0) = currentFrame.fx;
    K.at<float>(1,1) = currentFrame.fy;
    K.at<float>(0,2) = currentFrame.cx;
    K.at<float>(1,2) = currentFrame.cy;

    for (int i(0); i < mDB.mNumElem; i++){             //数据库中的历史20帧 这个数据库只维护20帧

        ORB_SLAM2::Frame refFrame = mDB.mvDataBase[i];
        cv::Mat bgr[3];
        cv::split(refFrame.mImRGB,bgr);                 //把参考帧 分为三层
        cv::Mat imR = bgr[2];
        cv::Mat imG = bgr[1];
        cv::Mat imB = bgr[0];

        cv::Mat vPixels(640*480,2,CV_32F);              //Mat(int rows, int cols, int type)
        cv::Mat mDepth(640*480,1,CV_32F);
        ...
        for (int j(0); j < 640*480; j++){
            int x = (int)vAllPixels.at<float>(j,0);
            int y = (int)vAllPixels.at<float>(j,1);
            if ((int)refFrame.mImMask.at<uchar>(y,x) == 1){                 //.mImMask这里的mImMaks是Yolact输出的mask.  有效区域
                
                const float d = refFrame.mImDepth.at<float>(y,x);           //
                if (d > 0){
                    vPixels.at<float>(n,0) = vAllPixels.at<float>(j,0);      //记录有效的位置
                    vPixels.at<float>(n,1) = vAllPixels.at<float>(j,1);
                    mDepth.at<float>(n,0) = 1./d;                            //记录有效位置的深度
                    n++;
                }
            }
        }
        ...
        vPixels = vPixels.rowRange(0,n);
        mDepth = mDepth.rowRange(0,n);
        hconcat(vPixels,cv::Mat::ones(n,1,CV_32F),vPixels);   //图片拼接
        ...
        // Divide by last column  齐次坐标 /4  归一化平面
        for (int j(0); j < vMPCurrentFrame.cols; j++)
        {
            vMPCurrentFrame.at<float>(0,j) /= vMPCurrentFrame.at<float>(3,j);
            vMPCurrentFrame.at<float>(1,j) /= vMPCurrentFrame.at<float>(3,j);
        ...
        for (int j(0); j < matCurrentFrame.cols; j++)
        {
            float x = matCurrentFrame.at<float>(0,j)/matCurrentFrame.at<float>(2,j);
            float y = matCurrentFrame.at<float>(1,j)/matCurrentFrame.at<float>(2,j);
            bool inFrame = (x > 1 && x < (currentFrame.mImDepth.cols - 1) && y > 1 && y < (currentFrame.mImDepth.rows - 1));  //判断是否出界  
            if (inFrame && (mask.at<uchar>(y,x) == 0))    //不仅不能出界 而且当前帧mask位置为空
            {
                vProjPixels.at<float>(p,0) = x;
                vProjPixels.at<float>(p,1) = y;
                _matProjDepth.at<float>(p,0) = matProjDepth.at<float>(0,j);
...
void Geometry::DataBase::InsertFrame2DB(const ORB_SLAM2::Frame &currentFrame){
    ...
    //这个数据库没有满的时候，则 mNumElem += 1;  用于记录数据库中有多少帧
    else {
        mvDataBase[mIni] = currentFrame;
        mFin = mIni;
        mIni = (mIni + 1) % MAX_DB_SIZE;
    }
    //如果是数据库里面的帧到达20张的时候，  则更新这20张  所以数据库中最多20张
}
