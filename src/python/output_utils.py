...
def postprocess(det_output, w, h, batch_idx=0, interpolation_mode='bilinear',
	...
	在测试模式下将Yolact的输出后处理成有意义的格式，
	     考虑所有可能的配置设置。

	     Args：
		 -det_output：检测输出的命令丢失。
		 -w：图像的实数。
		 -h：图像的实际高度。
		 -batch_idx：如果该批次有多个图像，则该图像在该批次中的索引。
		 -插值模式：可以为“最近” | '区域'| '双线性'（请参阅torch.nn.functional.interpolate）

	     返回4个割炬张量（按以下顺序）：
		 -classes [num_det]：每次检测的类idx。
		 -分数[num_det]：每次检测的置信度分数。
		 -框[num_det，4]：绝对值形式的每个检测的边界框。
		 -遮罩[num_det，h，w]：每次检测的完整图像遮罩。
